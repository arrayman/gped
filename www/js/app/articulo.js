...
com_arrayman_gped_art=
{
    ...
    dbget_arts: function(poArt,obs_artItem_added,obs_loaded)
    {
        poArt.list.length = 0;
        ...
        function on_getData(oData)
        {
           var len =  pck_Db.oResult.rows.length;
            ...
            for (var i=0; i<len; i++)
            {
                ...
                var oArt = new pck_Art.TART(poArt.list); // TArt(lthis.list);
                oArt.id = oitem.id; 
                oArt.codigo = oitem.codigo;
                oArt.descrip = oitem.desc;
                oArt.aux = oitem.aux;
                oArt.add2list();
